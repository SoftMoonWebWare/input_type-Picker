...
/*   The SoftMoon objec6 is usually a constant defined in a “pinnicle” file somewhere else
			...
			function getAdjacentPanelTabStop()  {
				return (
					...
					&&  ( isTabStop(tabTo)  //  ← see private members above  ↓                         ↓          ↓
						...
						|| (isTabStop(PickerInstance.dataTarget||PickerInstance.masterTarget)  &&  (tabTo=PickerInstance.dataTarget||PickerInstance.masterTarget))  ) );  }
			event.preventDefault();
					...
					if (typeof tabTo === 'string')  {
						...
						switch (tabTo.substring(0,1))  {
							...
							case '@': switch (tabTo.substring(1))  {
								...
								break;
								case 'target': tabTo= PickerInstance.dataTarget || PickerInstance.masterTarget;
								break;
								default: tabTo=null;  }
							break;
