// SoftMoon.WebWare.Picker.withDebug.js Beta-2.5.0 release 1.6.0  March-26-2015  by SoftMoon-WebWare.
...
SoftMoon.WebWare.Picker=function(mainPanel, opts)  {
	...
	if (typeof opts == 'object')  {
		...
		if (opts.debugLogger!==undefined)  {
			if (opts.debugLogger instanceof SoftMoon.WebWare.Log  ||  opts.debugLogger===window.console)
				this.debug=opts.debugLogger;
			else if (SoftMoon.WebWare.Log)
				this.debug=new SoftMoon.WebWare.Log(opts.debugLogger);
			else  throw new Error("invalid debugLogger");
			this.debug.eventTypes= (typeof opts.debugEvents == 'number') ?  opts.debugEvents  :  SoftMoon.WebWare.Picker.DEBUG_ALL;  }  }
...
//This is required to keep focus on the targetElement when the scroll-bars are clicked-on without blocking a click on the SoftMoon.WebWare.Picker.
...
SoftMoon.WebWare.Picker.prototype.setTopPanel=function(panel, rotate)  {
	...
	var i=0, z, tc, rc=RegExp('\\b(' + this.classNames.panelLevel + '[0-9]+|' + this.classNames.topPanel + '|' + this.classNames.activePanel + ')\\b', 'g');
	if (panel)  {
	...
	if (this.debug)  this.debug.log('======= set z-level and top/active panel classnames:');
		for (i=0; i<this.panels.length; i++)  {
			z=this.classNames.panelLevel+String(i+1);
			UniDOM.swapOutClass(this.panels[i], rc, z);
			if (this.debug)  this.debug.log(z+' :: '+this.panels[i].id+' :: '+this.panels[i].className);  }
		...
		if (this.debug)  this.debug.log(this.panels[i-1].id+' :: '+this.panels[i-1].className);  }
...
// when flag=false, target is a flag → true=“revert to masterTarget”  false=“retain dataTarget”
	...
	if (this.debug)  this.debug.log('oldActiveState: '+wasActive+' now: '+flag);
	...
	this.setTopPanel();
	if (this.debug)  this.debug.log('======= set active picker state classnames:');
	for (var i=0, PickerInstance=this;  i<this.panels.length;  i++)  {
		UniDOM.useClass(this.panels[i], this.classNames.activePicker, flag);
		if (this.debug)  this.debug.log(this.panels[i].id+' :: '+this.panels[i].className);
...
SoftMoon.WebWare.Picker.prototype.setActiveInterfaceState=function(flag, target, isInterfaceTarget)  {
	...
	if (this.debug && (this.debug.eventTypes & (SoftMoon.WebWare.Picker.DEBUG_CLICKS | SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS)))
		this.debug.log('oldInterfaseState: '+wasActive+' now: '+flag);
	...
	if (this.debug)  this.debug.log('======= set active interface state classnames:');
	for (var i=0, PickerInstance=this;  i<this.panels.length;  i++)  {
...
SoftMoon.WebWare.Picker.prototype.choosePicker=function(activeClasses, pickerName, flag)  {
	...
	if (pickers.length)  {
		...
		if (this.debug)  this.debug.log('======= set chosen-picker state classnames:');
		for (i=0; i<pickers.length; i++)  {
			...
			if (this.debug)  this.debug.log(pickers[i].id+' :: '+pickers[i].className);
...
SoftMoon.WebWare.Picker.prototype.pick=function(chosen)  {
	...
	if (this.debug)  this.debug.log('picking '+chosen);
...
SoftMoon.WebWare.Picker.prototype.registerPicker=function(picker)  {
	...
	UniDOM.addEventHandler(picker, 'onmouseover', function()  {
		if (this.debug && (this.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  this.debug.log('mouse over picker');
		PickerInstance.mouseOverPicker=this;  });
	UniDOM.addEventHandler(picker, 'onmouseout', function()  {
		if (this.debug && (this.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  this.debug.log('mouse out of picker');
		PickerInstance.mouseOverPicker=false;  });  }
...
SoftMoon.WebWare.Picker.prototype.registerTargetElement=function(element, PickerContainer)  {
	...
	UniDOM.addEventHandler(element, 'onfocus', function()  {
		if (this.debug && (this.debug.eventTypes & (SoftMoon.WebWare.Picker.DEBUG_ALL | SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS)))
			this.debug.log('onFocus for Target');
	...
	UniDOM.addEventHandler(element, 'onTabIn', function()  {
		if (this.debug && (this.debug.eventTypes & (SoftMoon.WebWare.Picker.DEBUG_ALL | SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS)))
			this.debug.log('onTabIn for Target');
		this.focus();  });
	UniDOM.addEventHandler(element, 'onblur', function()  {
		if (this.debug && (this.debug.eventTypes & (SoftMoon.WebWare.Picker.DEBUG_ALL | SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS)))
			this.debug.log('onBlur for Target');
/*
...
*/
	...
	UniDOM.addEventHandler(element, 'onMouseOver', function()  {
		if (this.debug && (this.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  this.debug.log('mouse over Target Element');
		PickerInstance.mouseOverDataTarget=this;  });
	UniDOM.addEventHandler(element, 'onMouseOut', function()  {
		if (this.debug && (this.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  this.debug.log('mouse out of Target Element');
		PickerInstance.mouseOverDataTarget=false;  });
...
SoftMoon.WebWare.Picker.prototype.registerInterfaceElement=function(element, actions)  {
	...
	UniDOM.addEventHandler(element, 'onMouseOver', function()  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))
			PickerInstance.debug.log('mouse over Interface Element “'+this.name+'”');
		PickerInstance.mouseOverInterfaceElement=this;  });
	UniDOM.addEventHandler(element, 'onMouseOut', function()  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))
			PickerInstance.debug.log('mouse off Interface Element “'+this.name+'”');
		PickerInstance.mouseOverInterfaceElement=false;  });

 if (isUserdataInputType)  {
	UniDOM.addEventHandler(element, 'onMouseOver', function()  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))
			PickerInstance.debug.log('mouse over UserData Interface Element “'+this.name+'”');
		PickerInstance.mouseOverUserdataInterface=this;  });
	UniDOM.addEventHandler(element, 'onMouseOut', function()  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))
			PickerInstance.debug.log('mouse off UserData Interface Element “'+this.name+'”');
		PickerInstance.mouseOverUserdataInterface=false;  });  }

 if (isUserdataInputType)
	UniDOM.addEventHandler(element, 'onclick', function(event)  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_CLICKS))
			PickerInstance.debug.log('Click on Interface Element “'+this.name+'”');
 ...
 if (isUserdataInputType)  {
	UniDOM.addEventHandler(element, 'onfocus', function(event)  {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Focusing Interface Element “'+this.name+'”');
	...
	UniDOM.addEventHandler(element, 'onTabIn', function(event) {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Tabbing into UserData Interface Element “'+this.name+'”');
	...
	UniDOM.addEventHandler(element, 'onTabIn', function(event) {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Tabbing into Interface Element “'+this.name+'”');
		PickerInstance.setActiveInterfaceState(true, this);
		var newFocus=this,
				thisPanel=UniDOM.getAncestorByClass(this, PickerInstance.classNames.pickerPanel);
		if (thisPanel)  PickerInstance.setTopPanel(thisPanel, event.rotatePanels);
	...
	UniDOM.addEventHandler(element, 'onkeydown', function(event)  {
		...
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Picker Interface Element — keydown  — keyCode= '+event.keyCode);
 ...
 if (isUserdataInputType)
	UniDOM.addEventHandler(element, 'onchange', function(event)  {
		...
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Picker Interface Element — onchange “'+this.name+'” — keyCode= '+key+' ¦ synFlag= '+event.flag);
	...
	UniDOM.addEventHandler(element, 'onkeyup', function()  {
		if (PickerInstance.event  &&  PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Picker Interface Element — keyup   — keyCode= '+PickerInstance.event.keyCode);
		PickerInstance.event=false;  });
 ...
 if (isUserdataInputType)
	UniDOM.addEventHandler(element, 'onblur', function(event)  {
		PickerInstance.event=false;
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS))
			PickerInstance.debug.log('Blurring interface element “'+this.name+'” — enterKeyed= '+enterKeyed+' ¦ tabbedOut= '+tabbedOut);
...
SoftMoon.WebWare.Picker.prototype.registerInterfacePanel=function(panel, actions)  {
	...
	UniDOM.addEventHandler(panel, 'onclick', function() {
		if (PickerInstance.debug  &&  (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_CLICKS))
			PickerInstance.debug.log('Click on interface panel');
		if (PickerInstance.pickerActiveFlag)  setTimeout(function()  { //help Google’s Chrome to keep track of what’s going on by setting a timeout (it has a hard time remembering to make the cursor blink)
			if (PickerInstance.debug && (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_CLICKS))  PickerInstance.debug.log('refocusing Target from click on picker panel');
	...
	UniDOM.addEventHandler(panel, 'onmouseover', function()  {
		if (PickerInstance.debug && (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  PickerInstance.debug.log('mouse over picker panel');
	...
	UniDOM.addEventHandler(panel, 'onmouseout', function() {
		if (PickerInstance.debug && (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  PickerInstance.debug.log('mouse out of picker panel');
	...
	if (panelBody)  for (i=0; i<panelBody.length; i++)  { if (panelBody[i].nodeType===Node.ELEMENT_NODE)  {
		...
		if (PickerInstance.debug && (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  PickerInstance.debug.log('mouse over picker panel body');
			PickerInstance.mouseOverPanelBody=this;  });
		UniDOM.addEventHandler(panelBody[i], 'onmouseout', function()  {
		if (PickerInstance.debug && (PickerInstance.debug.eventTypes & SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS))  PickerInstance.debug.log('mouse out of picker panel body');
...
SoftMoon.WebWare.Picker.DEBUG_ALL=0xFF;
SoftMoon.WebWare.Picker.DEBUG_INTERFACE_ELEMENTS=0x01;
SoftMoon.WebWare.Picker.DEBUG_MOUSEOVERS=0x02;
SoftMoon.WebWare.Picker.DEBUG_CLICKS=0x04;
